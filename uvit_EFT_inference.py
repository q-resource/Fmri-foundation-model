...
def cli_parser():
    ...
    parser.add_argument('--using_mask', default=True, type=bool)
    ...
    parser.add_argument('--using_seedfc',default=True,type=bool,help='using seedfc or not')
...
def load_data(args,region_mask_id):
    ...
    if args.using_mask:
        ...
        SSTBLmask=nib.load('/home/jiaty/EFTdataset/emotional_information_uniformity-test_z_FDR_0.01_3mm.nii.gz').get_fdata()
        SSTBLmask=SSTBLmask==0
        SSTFU2mask=nib.load('/home/jiaty/EFTdataset/emotional_information_uniformity-test_z_FDR_0.01_3mm.nii.gz').get_fdata()
        SSTFU2mask=SSTFU2mask==0
        SSTFU3mask = nib.load('/home/jiaty/EFTdataset/emotional_information_uniformity-test_z_FDR_0.01_3mm.nii.gz').get_fdata()
...
def inference_pretrained_model(args,frim_test_dataloader,save_path,device,region_mask_id):
    ...
    for ii in range(0, naverge):
        ...
        if args.region_mask_id is not None:
            np.save(os.path.join(save_path, "test_prediction_EFT_non_motor_control_naverge_{}_{}.npy".format(naverge,region_mask_id)), samples_all)
        else:
            np.save(os.path.join(save_path,"test_prediction_EFT_non_motor_control_naverge_{}.npy".format(naverge)),samples_all)
...
if __name__ == '__main__':
    ...
    setup_seed(34007)
    main_path="/home/jiaty/EFT_task"
    main_path_checkpoint = "/home/jiaty/EFT_task/checkpoint/"
    ...
    inference_log='prem_'+ args.pred_mode +  'bs_' + str(args.batch_size)+'ps_' + str(args.patch_size) + 'dep_' + str(
        args.depth) + 'embd_' + str(args.embed_dim) + 'nh_' + str(args.num_heads) + 'epo_' + str(args.nepoch) + 'lr_' +str(args.lr)\
              ...
              +"_using_vbm"+str(args.using_vbm)+"cfg_"+str(args.cfg_scale)+"_nstep_"+str(args.nstep)+"_inference_mode:"+str(args.inference_mode)
